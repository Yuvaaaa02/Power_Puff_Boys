...
export default function DashboardClient({ params }: { params: any }) {
  ...
  const fetchData = async (month: string) => {
    try {
      ...
      const [expRes, setRes, usrRes] = await Promise.all([
        fetch(apiUrl(`/api/expenses?month=${month}&t=${ts}`)),
        fetch(apiUrl(`/api/settlements?month=${month}&t=${ts}`)),
      ...
      const ts2 = Date.now();
      const allExpRes = await fetch(apiUrl(`/api/expenses?t=${ts2}`));
      const allExpData: Expense[] = await allExpRes.json();
      const allSetRes = await fetch(apiUrl(`/api/settlements?t=${ts2}`));
